...
2. **数据迁移（Rehash）**：JDK 1.8 做了一个极其优雅的优化。因为容量翻倍，节点在新数组中的位置只有两种可能：
   ...
   - 底层通过高位运算 `(e.hash & oldCap) == 0` 来以 $O(1)$ 的速度决定节点去哪，且不再需要重新计算 Hash 值。
   - 除此以外，它将JDK1.7扩容时的头插法换成了尾插法，这样可以保证链表节点的相对顺序，避免了多进程并发扩容引起的环形链表死循环。
 ...
 **面试官追问 1：“CAS 是怎么保证‘比较和交换’这两个动作不被别人打断的？”**

> **✅ 高分回答（直击底层硬件）：** “CAS 的原子性不是靠 Java 代码保证的，而是靠**硬件级别的指令**保证的。 在 Java 中，CAS 是通过 `sun.misc.Unsafe` 类里的一系列 `native` 方法调用的。当这些方法编译成底层 CPU 指令时，在 x86 架构下会对应一条带 `lock` 前缀的指令（如 `lock cmpxchg`）。 这个 `lock` 前缀在多核处理器下会触发**缓存锁定（或总线锁定）**，它能保证在执行这个指令的期间，其他 CPU 核心绝对无法修改这块内存区域的数据，从而在硬件层面实现了绝对的原子性。”
...
> “Java 的泛型是不变的，`List<String>` 不是 `List<Object>` 的子类，直接强转会导致堆污染（Heap Pollution），从而在运行时触发类型转换异常。如果只是为了兼容方法传参，我们可以使用无界通配符 `List<?>` 或上界通配符 `List<? extends Object>`；如果必须得到一个新的 List，则需要使用 Stream API 开辟新集合进行映射拷贝。”
>
> ```
> List<String> strList = Arrays.asList("A", "B");
> List<Object> objList = strList.stream()
>                               .map(s -> (Object) s) // 向上转型，绝对安全
>                               .collect(Collectors.toList());
>                       
> List<Object> objList = Arrays.asList("A", "B");
> List<String> strList = objList.stream()
>                               .map(String.class::cast) // 强转，如果里面混了数字会抛异常
>                               .collect(Collectors.toList());
> ```
>
> 
...
`? extends Fruit` 意思是：这个集合里装的**全都是 Fruit 或者 Fruit 的子类**。

`? super Apple` 意思是：这个集合里装的**全都是 Apple 或者 Apple 的父类**。

> | **通配符类型**                   | **操作类型**       | **具体动作 (代码)**                                          | **结果**   | **编译器的底层逻辑 (面试核心话术)**                          |
> | -------------------------------- | ------------------ | ------------------------------------------------------------ | ---------- | ------------------------------------------------------------ |
> | **`? super Apple`** (只写不读)   | **1. 写父类**      | `superList.add(new Fruit())`                                 | ❌ **报错** | 篮子真身可能是 `List<Apple>`，装不下其他水果。               |
> |                                  | **2. 写本类/子类** | `superList.add(new RedApple())`                              | ✔️ **安全** | 篮子最差也是装 `Apple` 的，往里扔红富士绝对合法 (多态向上转型)。 |
> |                                  | **3. 读父类**      | `Fruit f = superList.get(0)`                                 | ❌ **报错** | 篮子真身可能是 `List<Object>`，拿出来的可能是个 String，不能当水果卖。（只能用 `Object` 接） |
> |                                  | **4. 读子类**      | `RedApple r = superList.get(0)`                              | ❌ **报错** | 篮子里装的可能是普通的 `Apple`，不能强行当红富士卖。         |
> | ---                              | ---                | ---                                                          | ---        | ---                                                          |
> | **`? extends Apple`** (只读不写) | **5. 写父类**      | `extendsList.add(new Fruit())`                               | ❌ **报错** | 篮子真身可能是 `List<RedApple>`，你不能往里塞普通的香蕉或水果。 |
> |                                  | **6. 写本类/子类** | `extendsList.add(new RedApple())`                            | ❌ **报错** | **(极易踩坑)** 篮子真身可能是 `List<GreenApple>`！红绿不容，编译器不知道具体是哪种苹果篮，干脆**禁止一切写入** (只能写 `null`)。 |
> |                                  | **7. 读本类/父类** | `Fruit f = extendsList.get(0)` `Apple a = extendsList.get(0)` | ✔️ **安全** | 无论真身是红苹果还是青苹果，拿出来的**绝对是个 Apple**，自然也是个 Fruit。 |
> |                                  | **8. 读子类**      | `RedApple r = extendsList.get(0)`                            | ❌ **报错** | 拿出来的不一定是红富士，可能是青苹果，向下转型不安全。       |

**如果面试官问：“你讲讲 Java 泛型里的 PECS 原则是什么？为什么要这么设计？”**

> 你可以直接这样“降维打击”：
>
> > “PECS 就是 Producer Extends, Consumer Super。它是 Java 泛型为了解决**集合的协变和逆变**问题而引入的上下界通配符机制。
> >
> > 如果我们需要一个频繁**提供数据**的只读集合，就用 `? extends T`，它保证了拿出来的元素至少是 T 类型，但为了防止堆污染，编译器严格禁止往里添加元素。 反之，如果我们需要一个频繁**接收数据**的只写集合，就用 `? super T`，它允许我们安全地写入 T 及其子类对象，但读取时类型会丢失，只能用 Object 接收。
> >
> > 在实际的 JDK 源码中，比如 `Collections.copy(List<? super T> dest, List<? extends T> src)` 这个方法，就把 PECS 原则用到了极致：源列表 `src` 是生产者，只提供数据；目标列表 `dest` 是消费者，只接收数据。这种设计在保证绝对类型安全的同时，赋予了 API 极大的灵活性。”
...
> 如果面试官问：“你讲讲对 Java 反射的理解，以及在实际项目里的应用？”
>
> 你直接这么抛给他（绝对高分）：
>
> > “反射的本质是在运行期间动态获取类的结构，并操作对象的属性和方法。
> >
> > 在我的实际开发中，虽然很少直接写底层的 Reflection API，但整个项目基石都是反射。 比如在 Spring 中，我使用 `@Service` 配合 `@RequiredArgsConstructor`，Spring 底层就是通过反射获取类的带参构造器，动态实例化并注入 `private final` 依赖的。 再比如 MyBatis 处理数据库映射，我的实体类 DO 字段全是 `private`，MyBatis 底层就是通过反射的 `setAccessible(true)` 打破封装，把 ResultSet 的结果强行注入到 Java 对象的私有属性中，包括我配置的 `MyMetaObjectHandler` 实现审计字段（如 `createTime`）自动填充，底层也是依赖反射机制根据字符串动态寻址并赋值的。”
...
Map<Long, User> userMap = userList.stream()
    .collect(Collectors.toMap(
        User::getId,                  // 第 1 个参数：怎么提取 Key（用 id）
        user -> user,                 // 第 2 个参数：怎么提取 Value（用对象本身，也可以写 Function.identity()）
        (oldValue, newValue) -> newValue // 第 3 个参数：核心！遇到重复 Key 时怎么办？（新值覆盖旧值）
    ));
```

面试手撕这几个 API 确实容易让人心里发毛。平时在 IDEA 里敲个 `.` 就有代码提示，真到了白板或者网页评测系统里，稍微忘了一个 `Collectors` 的拼写或者 `Comparator` 的用法就会卡住。

不过别慌，在真实的 Java 后端面试中，手撕 Stream 和 Optional 翻来覆去考的就是那么几个**固定的业务模型**。我帮你精选了 **3 个大厂面试最高频的实战场景**，并配上字符画，你把这三个例子背后的逻辑盘明白，面试基本就能横着走了。

------

### 场景一：复杂的分组与统计 (Stream `groupingBy`)

**面试官出题：** “我现在有一个订单列表 `List<Order>`，请你帮我按 `userId` 分组，并且计算出每个用户购买的总金额。”

这是企业级开发中最典型的“数据聚合”场景，不用 Stream 的话要写一堆 `Map` 和 `for` 循环。
...
```java
// 假设实体类：Order(Long userId, BigDecimal amount)

Map<Long, BigDecimal> userAmountMap = orderList.stream()
    .collect(Collectors.groupingBy(
        Order::getUserId,                               // 第 1 个参数：按什么分组？(userId)
        Collectors.reducing(                            // 第 2 个参数：分组后怎么聚合？
            BigDecimal.ZERO,                            // 初始值
            Order::getAmount,                           // 取出金额
            BigDecimal::add                             // 怎么累加？(求和)
        )
    ));
```

### 场景二：多字段复杂排序与 Top N (Stream `sorted` + `limit`)

**面试官出题：** “给你一个学生列表 `List<Student>`，请你先按【成绩降序】排，如果成绩相同，再按【年龄升序】排，最后只取前 3 名学生的名字。”

这个考点极其经典，专门考察你对 `Comparator` 链式调用和终端操作的熟练度。

Java

```
// 假设实体类：Student(String name, int score, int age)

List<String> top3Names = studentList.stream()
    // 核心考点：多条件联合排序
    .sorted(Comparator.comparing(Student::getScore).reversed() // 成绩降序
                      .thenComparing(Student::getAge))         // 年龄升序 (默认)
...
在写这段代码时，你可以主动和面试官提一句：“这里 `limit(3)` 是一个**短路操作（Short-circuiting）**，配合 Stream 的惰性求值特性，底层引擎在拿到 3 个结果后就会提前结束流水线，不用处理完所有数据，效率很高。”

------

### 场景三：恐怖的级联判空 (Optional `map` vs `flatMap`)

**面试官出题：** “在业务代码里，经常会出现 `user.getAddress().getCity().getName()` 这种深度嵌套的调用。只要中间任何一环是 null，就会报 NPE（空指针）。请用 `Optional` 优雅地重构它。”
...
这是防雷必备基操。面试官不仅看你会不会用 `Optional`，还可能顺带考你 `map` 和 `flatMap` 的区别。
...
[📦 盒子: User]
   │  .map(User::getAddress)
   ▼  (如果 User 是 null，盒子直接变空，后面的逻辑全跳过)
[📦 盒子: Address]
   │  .map(Address::getCity)
   ▼  (哪怕 Address 没填，安全跳过，不会爆 NPE)
[📦 盒子: City]
   │  .map(City::getName)
   ▼
[📦 盒子: "北京" 或 空]
...
**🚨 面试官进阶追问：“如果 `getAddress()` 方法本身的返回值就是一个 `Optional<Address>`，你还能用 `map` 吗？”**

> **你的高分回答：** “不行了！如果原来方法的返回值本身就是 Optional，再用 `map` 会导致套娃，变成 `Optional<Optional<Address>>`。这时候必须用 **`flatMap`**，它就像一个‘压平机’，能把两层包装盒压成一层，保持流水线的清爽。”
